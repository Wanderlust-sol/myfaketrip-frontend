import React, { Component } from "react";
import { MdStar, MdStarBorder, MdStarHalf } from "react-icons/md"
import "./Product.scss"

class Product extends Component {
    constructor(props) {
        super(props);
        
      }

    render() {
        const stars = [];
        const getStars = rating => {
            if(rating !== Math.floor(rating)) {
                for (let i = 0; i < Math.floor(rating); i++) {
                    stars.push(<MdStar className="star_blue"/>);
                }
                for (let i = 0; i < Math.ceil(rating) - Math.floor(rating); i++) {
                    stars.push(<MdStarHalf className="star_blue"/>);
                }
                for (let i = 0; i < 5 - Math.ceil(rating); i++) {
                    stars.push(<MdStarBorder className="star_blue"/>);
                }
            } else {
                for (let i = 0; i < Math.floor(rating); i++) {
                    stars.push(<MdStar className="star_blue"/>);
                }
                for (let i = 0; i < 5 - Math.floor(rating); i++) {
                    stars.push(<MdStarBorder className="star_blue"/>);
                }
            }
            return stars;
        };
        

        return (
            <div className="product_container">
                <div className="product_img">
                    <img src={this.props.data.thumnail} alt=""/>
                    <div className="wishlist">
                        <img src="data:image/svg+xml;base64," alt="wishlist" role="button"/>
                    </div>
                </div>
                <div className="product_cnt">
                    <div className="category">
                        <span>{this.props.data.product_type}</span>
                        <span> ・ </span>
                        <span>{this.props.data.city_name}</span>
                    </div>
                    <h3 className="product_title">
                        {this.props.data.product_name}
                    </h3>
                    <div className="product_info">
                        <div className="review">
                            <div>
                                {getStars(this.props.data.grade)}
                            </div>
                            <span>41</span>
                        </div>
                        <div className="price">
                            <span className="price_origin" style={{marginRight: this.props.data.origin_price ? "4px" : "0px"}}>
                                {this.props.data.origin_price
                                ? this.props.data.origin_price.toLocaleString() + "원" 
                                : null}
                            </span>
                            <span className="price_main">
                                {this.props.data.price.toLocaleString()}
                                <span>원</span>
                            </span>
                            <span className="price_standard">/ 1인</span>
                        </div>
                    </div>
                </div>
            </div>
        )
    }
}


export default Product;